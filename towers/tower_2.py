...
class Tower_2(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        self.bullet_damage = TOWER_2_BULLET_DAMAGE

    def update(self, enemies, player):
        now = pygame.time.get_ticks()
        
        enemy_detected = check_collision_group_circle(self, enemies, TOWER_2_DETECTION_RADIUS)
        #player_detected = check_collision_circle(self, player, TOWER_2_DETECTION_RADIUS)
        if enemy_detected != False:
            ...
            if enemy_detected != False and now > self.spawn_bullet_timer:
                ...
                self.spawn_bullet_timer = now + TOWER_2_BULLET_TIMER
        else:
            self.highlight_detection_circle = False

        for bullet in self.bullets:
            bullet.update()
            if(not check_collision_circle(self, bullet, TOWER_2_DETECTION_RADIUS)):
               bullet.kill()

    def draw(self, surface):
        if self.highlight_detection_circle:
            circle_center = (self.rect.centerx, self.rect.centery)
            pygame.draw.circle(surface, GREEN, circle_center, TOWER_2_DETECTION_RADIUS, 2)
...
class TowerBullet(pygame.sprite.Sprite):
    def __init__(self, tower, angle):
        ...
        self.image.fill(TOWER_2_BULLET_COLOR)
